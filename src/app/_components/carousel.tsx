...
const Modal = memo(({ isOpen, onClose, children, currentIndex, totalItems, onPrev, onNext, photos, setCurrentIndex }: {
  ...
  return (
    ...
    >
      ...
      {/* Image container */}
      <div className="relative w-full h-full flex items-center justify-center p-4 sm:p-8 pb-20 sm:pb-24">
        {children}
      </div>

      {/* Thumbnail strip — scrollable row so it never overflows on small screens */}
...
const ImageCarousel = memo(({ photos }: CarouselProps) => {
  ...
  // Re-show swipe hint whenever the photo set changes (new restaurant loaded)
  useEffect(() => {
    setCurrentIndex(0);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        onClick={openModal}
      >
        {/* Initial swipe hint animation for mobile — only shown briefly */}
        {showSwipeHint && (
        ...
        {/* Swipe indicators — only visible during active swipe */}
      ...
      {/* Navigation Arrows
          On mobile (< sm): always visible at 70% opacity for discoverability.
          On sm+: hidden by default, appear on group-hover / focus. */}
      <button
        onClick={(e) => {
          e.stopPropagation();
          prevSlide();
